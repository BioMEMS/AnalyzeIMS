...
def train_model(model, criteria, optimizer, lr_scheduler, num_epochs):
    since = time.time()
    early_stop_count = 0
        ...
        for phase in ['train', 'val']:
            ...
            for i in range(batches_per_epoch):
                ...
                if phase == 'val':
                    ...
                    for i in np.arange(num_augs):
                        try:
                            inputs_temp = preprocess_val(inputs_raw)
                            inputs = torch.cat([inputs, inputs_temp], dim=0)
                            labels_temp = torch.tensor(test_y[start:start + batch_size]).long()
                            labels = torch.cat([labels, labels_temp], dim=0)
                        except:
                            labels = torch.tensor(test_y[start:start + batch_size]).long()
                            inputs = preprocess_val(inputs_raw)
            ...
            if phase == 'val':
                ...
                if epoch_acc > best_acc:
                    ...
                    print('\n')
                    print('\n')
                    print('EARLY STOP COUNT IS ', early_stop_count)
            if phase == 'train':
                if epoch_acc > best_acc:
                    best_acc = epoch_acc
                    best_model = copy.deepcopy(model)
...
model_ft = train_model(model, criteria, optimizer_ft, exp_lr_scheduler,
                       num_epochs=100)
