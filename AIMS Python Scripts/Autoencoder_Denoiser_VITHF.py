...
custom_width, custom_height = 640, 640
num_channels = 3
...
def unpatchify(x, patch_size, img_size, num_channels):
    """
    ...
    x = x.reshape(N, h, w, patch_size, patch_size, num_channels)
    x = x.permute(0, 5, 1, 3, 2, 4)  # (N, C, h, patch, w, patch)
    x = x.reshape(N, num_channels, img_size[0], img_size[1])
    return x
...
new_pos_embed = nn.Parameter(torch.zeros(1, num_patches+1, hidden_size))
nn.init.trunc_normal_(new_pos_embed, std=0.02)
model.vit.embeddings.position_embeddings = new_pos_embed
model.vit.embeddings.num_channels = 3
model.vit.embeddings.patch_embeddings.num_channels = 3
...
recon = unpatchify(reconstructed_pixel_values, patch_size[0], (custom_width, custom_height), num_channels)
...
reconstructed_image = recon.detach().clip(0, 1)
...
plt.imshow(reconstructed_image[0,1,:,:].squeeze(), cmap="gray")
plt.title("Reconstructed Image")
plt.axis("off")
plt.show()

plt.imshow(reconstructed_image[0,2,:,:].squeeze(), cmap="gray")
plt.title("Reconstructed Image")
plt.axis("off")
plt.show()